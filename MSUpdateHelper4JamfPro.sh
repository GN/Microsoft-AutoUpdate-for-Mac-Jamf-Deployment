...
PATH_WATP="/Applications/Microsoft Defender ATP.app"
...
function PerformUpdate() {
    Debug "PerformUpdate: ${CMD_PREFIX}/Library/Application\ Support/Microsoft/MAU2.0/Microsoft\ AutoUpdate.app/Contents/MacOS/msupdate --install --apps $1 $2 --wait 600"
	${CMD_PREFIX}/Library/Application\ Support/Microsoft/MAU2.0/Microsoft\ AutoUpdate.app/Contents/MacOS/msupdate --install --apps $1 $2 --wait 600 | tee /tmp/$1.log
    
    # tells us if MSAU is updating, and if it is sleep to give it time to finish.  
    cat /tmp/$1.log | grep MSau04
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
      echo "Sleeping for 300 seconds to wait for MSau04 update"
      sleep 300
    fi
    rm /tmp/$1.log
}
...
if [ "$UPDATE_WATP" == "true" ]; then
	Debug "Going for Windows ATP update"
	RegisterApp "$PATH_WATP" "WDAV00"
	SetTargetVersion "$VERSION_WATP"
	PerformUpdate "WDAV00" "$TARGET_VERSION"
else
	Debug "Update for Windows ATP disabled"
fi
exit 0
